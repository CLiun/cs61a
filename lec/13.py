...
def partition_tree(n, m):
    if n == 0:
        ...
        # Left tree use m at least once, right tree use m - 1
        left_branch = partition_tree(n - m, m)
        right_branch = partition_tree(n, m - 1)
        return tree(m, [left_branch, right_branch])
    
def print_partition(t, partition=[]):
    if is_leaf(t):
        if label(t):
